...
export default function CardCobrancasVencidas() {
    const {
        ...
        setCobrancasVencidasPerduram, setTodosClientes
    } = useGlobal();


    useEffect(() => {
        async function cobrancas_Fetch() {
            const { cobrancas_Vencidas } = await buscar_Dados_Cobrancas(
                token,
                setListaCobrancasVencidas);
            ...
            await setCobrancasVencidasPerduram([...itensCobrancasVencidas]);
        }
        cobrancas_Fetch();
    ...
    function ordenarClientes() {
        const arrayDeCobrancas = [...lista_Vencidas];
        ...
        setListaCobrancasVencidas(arrayDeCobrancasOrdenado);
    }
    ...
    return (
        <>
            <div className='container-card_tabela-cobrancas'>
                ...
                {lista_Vencidas && lista_Vencidas.slice(parametroSlice, parametroSlice + 10).map(cobranca => (
                    <div
                ...
                <div
                    ...
                    <div className='paginacao-cobrancas-direita'
                        onClick={() => {
                            if (parametroSlice + 10 >= lista_Vencidas.length) return;
                            setParametroSlice(parametroSlice + 10);
                        }}>
                        <h1>{'>>>>'}</h1>
                    </div>
